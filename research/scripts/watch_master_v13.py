...
def build_master_packet(i: int, i2: int, opcode: int, payload: bytes) -> list[bytes]:
    ...
    Returns a list of 20-byte BLE packets ready to transmit.
    """
    global pid_counter
    length = len(payload)
    
    if length <= 10:
        # Single fragment (20 bytes total)
        pkt = bytearray(20)
        pkt[0] = 0x00               # Fragment #0
        pkt[1] = pid_counter & 0xFF # Packet ID
        pkt[2] = 0x00               # Additional fragments = 0
        pkt[3] = i & 0xFF           # Category (usually 0)
        pkt[4] = i2 & 0xFF          # Subcategory (usually 1)
        pkt[5] = opcode & 0xFF      # Command Opcode!
        pkt[6] = 0x00               # Reserved Padding
        pkt[7] = 0x00               # Reserved Padding
        pkt[8:10] = struct.pack("<H", length) # Payload Length (Little-Endian LSB)
        pkt[10:10+length] = payload
        
        pid_counter = (pid_counter + 1) % 256
        return [bytes(pkt)]
    else:
        # Multi-fragment message (for AI Text, Alarms, etc.)
        remaining = length - 10
        ...
        buffer[2] = add_frags & 0xFF # Number of additional chunks following!
        buffer[3] = i & 0xFF
        buffer[4] = i2 & 0xFF
        ...
        buffer[10:20] = payload[:10] # First 10 bytes of payload
        
        # Subsequent Fragments (19 bytes of payload per 20-byte chunk)
        for f in range(add_frags):
            offset = (f + 1) * 20
            buffer[offset] = f + 1   # Fragment Index #1, #2, #3...
...
def get_time_sync_payload() -> bytes:
    """ProtocolAppToDevice.e(int i, long j): 4B Unix Sec + 4B TZ Offset + 1B Flag"""
...
def get_ai_text_payload(text: str) -> bytes:
    """ProtocolAppToDevice.a(int i, String str): 9B Header + UTF-8 String"""
...
async def send_command(chunks: list[bytes], label: str = "Command"):
    ...
    try:
        for idx, chunk in enumerate(chunks):
            ...
            if len(chunks) > 1:
                await asyncio.sleep(0.05) # 50ms interval between fragments
...
async def run_automated_test():
    ...
    for i in range(1, 4):
        print(f"\n--- Firing Buzz #{i} ---")
        # g(0, 1, 116 / 0x74, payload 0x01)
        buzz_chunks = build_master_packet(0, 1, 0x74, b"\x01")
        await send_command(buzz_chunks, f"Vibrate Motor Pulse #{i}")
